...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bonsaicoin"))
        return false;
...
{
    // Convert bonsaicoin:// to bonsaicoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bonsaicoin://"))
    {
        uri.replace(0, 12, "bonsaicoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bonsaicoin.lnk";
}
...
{
    return GetAutostartDir() / "bonsaicoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bonsaicoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bonsaicoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Bonsaicoin-Qt"));
